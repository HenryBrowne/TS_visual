...
from app.datasets import models as dataset_models  # noqa: F401  (registers Dataset, needed for the FK)
...
def test_upsert_inserts_new_entry(db_session):
    ...
    rows = get_leaderboard_rows(db_session, dataset_id="ds1")
    assert len(rows) == 1
...
def test_upsert_updates_existing_entry_rather_than_duplicating(db_session):
    ...
    upsert_leaderboard_entry(
        ...
        series_id="D1",
        model_name="ARIMA",
        metrics={"mae": 5.0, "rmse": 6.0, "smape": 7.0},
        job_id="job2",
    )
    db_session.commit()
...
def test_leaderboard_aggregates_per_model_across_series_and_sorts_by_smape(db_session):
    upsert_leaderboard_entry(
        ...
        series_id="D1",
        model_name="ARIMA",
        metrics={"mae": 10.0, "rmse": 12.0, "smape": 5.0},
        job_id="j",
    )
    upsert_leaderboard_entry(
        db_session,
        ...
        series_id="D2",
        model_name="ARIMA",
        metrics={"mae": 20.0, "rmse": 22.0, "smape": 15.0},
        job_id="j",
    )
    upsert_leaderboard_entry(
        db_session,
...
def test_leaderboard_does_not_mix_across_datasets(db_session):
    # Same series_id and model_name in two different datasets must not collide
    ...
    upsert_leaderboard_entry(
        ...
        series_id="D1",
        model_name="ARIMA",
        metrics={"mae": 99.0, "rmse": 99.0, "smape": 99.0},
        job_id="j",
    )
    db_session.commit()

    ds1_rows = get_leaderboard_rows(db_session, dataset_id="ds1")
    ds2_rows = get_leaderboard_rows(db_session, dataset_id="ds2")

    assert ds1_rows[0]["avg_mae"] == 1.0
    assert ds2_rows[0]["avg_mae"] == 99.0
